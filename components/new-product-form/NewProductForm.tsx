...
export default function NewProductForm({ categories, materials, states, colors }: Props) {
  ...
  return (
    <>
      ...
      <main className="w-full min-h-screen lg:hidden bg-[#f0f2f5] dark:bg-dark3 pt-6 pb-16 lg:pt-12 flex gap-8 lg:gap-12 flex-col items-center justify-center relative">
        {/* Header: Ajuste de tamaño de texto para mobile */}
        <div className="flex flex-col items-center px-4 text-center">
          <h2 className="text-base tracking-wider lg:text-xl text-sky-500 dark:text-[hsl(41,98%,65%)]  uppercase font-semibold">
            Gestión de productos
          </h2>
          <h1 className="text-3xl lg:text-6xl relative mt-2">
            Crear Nuevo Producto
            <span className="absolute w-12 lg:w-20 border-b-4 bottom-0 left-[calc(50%-24px)] lg:left-[calc(50%-40px)] border-sky-500 dark:border-[hsl(41,98%,65%)] rounded-xl"></span>
          </h1>
        </div>
        ...
        <form
          ref={formRef}
          action={handleSubmit}
          /* CAMBIO CLAVE: De block/flex-col en mobile a grid en desktops grandes */
          className="w-full max-w-4xl px-4 flex flex-col lg:grid lg:grid-cols-[1fr_280px] gap-6 items-start dark:bg-dark3"
        >
          {/* ── Columna izquierda (Contenido Principal) ── */}
          <div className="flex flex-col gap-6 w-full order-2 lg:order-1">
            {/* Product Identity */}
            <div className="bg-white rounded-2xl border border-gray-200 dark:border-black p-5 lg:p-6 dark:bg-dark3">
              <h2 className="flex items-center gap-2 text-lg lg:text-xl font-medium mb-5 text-gray-800 tracking-wider dark:text-white">
                <PenLine
                  size={16}
                  className="text-sky-500 dark:text-[hsl(41,98%,65%)]"
                />
              ...
              <div className="flex flex-col gap-4">
                <div>
                  <label className="text-[10px] lg:text-[11px] tracking-widest text-gray-400 dark:text-gray-200 uppercase block mb-1">
                    Título
                  </label>
                  <input
                    name="title"
                ...
                <div>
                  ...
                  <textarea
                    name="description"
                    rows={5}
                    placeholder="Describe the mechanical properties..."
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-sky-400 dark:focus:border-[hsl(41,98%,65%)] transition-colors resize-none"
                  />
                </div>
              </div>
            ...
            {/* Technical Specifications */}
            <div className="bg-white rounded-2xl border border-gray-200 dark:border-black p-5 lg:p-6 dark:bg-dark3">
              <h2 className="flex items-center gap-2 tracking-wider text-lg lg:text-xl font-medium mb-5 text-gray-800 dark:text-gray-200">
                <Grid2x2
                  size={16}
                  className="text-sky-500 dark:text-[hsl(41,98%,65%)]"
                />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="col-span-1 md:col-span-2 lg:col-span-1">
                  <label className="text-[10px] lg:text-[11px] tracking-widest text-gray-400 dark:text-gray-200 uppercase block mb-2">
                    Dimensions (cm)
                  </label>
                ...
                {/* Selects: Se apilan en móvil, 2 columnas en desktop */}
                <div className="flex flex-col gap-4">
                  <label className="text-[10px] lg:text-[11px] tracking-widest text-gray-400 dark:text-gray-200 uppercase block">
                    Categoría
                  </label>
                  <div className="flex gap-2">
                    <select
                      name="category_id"
                      className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm dark:bg-dark2/65 bg-white"
                    >
                      <option value="">Elige una Categoría</option>
                      {categoryList.map((c) => (
                        <option key={c.id} value={c.id}>
                    ...
                    <button
                      ...
                      className="w-10 h-10 shrink-0 flex items-center justify-center rounded-lg border border-gray-200"
                    >
                      <Plus size={16} />
                    </button>
                  </div>
          ...
          <div className="flex flex-col gap-6 w-full order-1 lg:order-2">
            <div className="bg-white dark:bg-dark3 rounded-2xl border border-gray-200 dark:border-black p-5 flex flex-col gap-6">
              {/* Imagen: Más pequeña en mobile para no ocupar toda la pantalla */}
              <div
                onClick={() => fileInputRef.current?.click()}
                className="aspect-square lg:aspect-square w-full max-w-[240px] mx-auto lg:max-w-none bg-gray-100 dark:bg-dark2 rounded-xl flex items-center justify-center border border-dashed border-gray-300 cursor-pointer overflow-hidden relative"
              >
                {badge !== "None" && (
                  <div className="absolute top-2 left-2 z-10 bg-sky-400 dark:bg-[hsl(41,98%,65%)] text-sky-900 dark:text-white text-[10px] font-medium px-2 py-1 rounded-md uppercase">
                    {badge}
                  </div>
                )}
                {imagePreview ? (
              ...
              <div className="space-y-6">
                <div>
                  <label className="text-[10px] tracking-widest text-gray-400 dark:text-gray-200 uppercase block mb-3">
                    Etiqueta
                  </label>
                  <div className="flex flex-wrap gap-2">
                    {BADGES.map((b) => (
                      <button
                        ...
                        type="button"
                        onClick={() => setBadge(b)}
                        className={`text-[10px] px-3 py-1.5 rounded-full border transition-colors ${badge === b ? "bg-sky-400 text-sky-900 border-sky-400" : "border-gray-200 text-gray-500"}`}
                      >
                        {b}
                      </button>
                    ))}
                  ...
                  <div className="flex flex-wrap gap-3 items-center">
                    {colorList.map((c) => (
                      <button
                        ...
                        onClick={() => toggleColor(c.id)}
                        style={{ backgroundColor: `#${c.hex_code}` }}
                        className={`w-8 h-8 rounded-full border border-black/5 ${selectedColors.includes(c.id) ? "ring-2 ring-offset-2 ring-gray-800 scale-110" : ""}`}
                      />
                    ))}
                    <button
                      type="button"
            ...
            {/* Botón de Publicar: Siempre visible al final o arriba en mobile */}
            <div className="bg-white dark:bg-dark3 rounded-2xl border border-gray-200 dark:border-black p-5">
              <button
                type="submit"
                disabled={loading}
                className="w-full py-4 bg-sky-400 hover:bg-sky-500 dark:bg-[hsl(41,98%,65%)] dark:text-white text-sky-900 font-bold text-sm rounded-xl tracking-[0.2em] disabled:opacity-50 transition-all active:scale-[0.98]"
              >
          ...
          {/* Modales (se mantienen igual, suelen ser responsivos por defecto si AddOptionModal está bien hecho) */}
